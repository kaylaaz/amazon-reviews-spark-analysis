...
*Figure 1. Spark UI captured during data loading, which shows multiple active executors on SDSC Expanse, confirming the job ran in a distributed configuration rather than locally.*
...
Out of the 109,830,520 total rows, 104,582,187 `review_id`s are unique, leaving 5,248,333 duplicate review IDs (about 4.8%).
...
Our target is `helpfulness_ratio`, which is defined as `helpfulness_ratio = helpful_votes / total_votes`. It is computed only for rows where `total_votes > 0` to avoid division by zero. For modeling, this is binarized into a `label` (1.0 if ratio is at least 0.5, else 0.0).
...
`product_category` was filtered to a predefined list of valid category names to remove date and text corruption. `star_rating`, `helpful_votes`, and `total_votes` were cast to integers, and rows with star ratings outside 1 to 5 were dropped. Rows with nulls in essential columns (`review_id`, `review_body`, `review_headline`, `review_date`, `product_category`) were dropped, duplicates were removed on `review_id` to prevent leakage between splits, and a median `Imputer` handled any remaining missing numeric values.
...
`review_length` was computed as the character count of the review body. A `Tokenizer` produced `review_tokens` to enable possible text-based expansion. The logistic regression baseline applies `StandardScaler` within its own pipeline.
...
The second model applies PCA (`pyspark.ml.feature.PCA`) to the same five engineered features, followed by Logistic Regression. The pipeline has four stages: `VectorAssembler`, then `StandardScaler` (withMean=True, withStd=True), then `PCA(k=3)`, then `LogisticRegression`. The same 70/15/15 split (seed=42) and feature set from Model 1 were reused so that any performance difference isolates the effect of dimensionality reduction rather than data or feature differences.
...
feature_cols = ["star_rating", "review_headline_length", "verified_purchase_idx",
                "category_idx", "log_review_length"]
...
PCA was configured with `k=3` (three principal components from the five standardized features) to cover the majority of variance (70.9%) while achieving meaningful compression. Standardization before PCA is required so that features with larger numeric ranges do not dominate the principal components. After fitting, explained variance and component loadings were extracted from `model.stages[2]`, and predictions were generated on the train, validation, and test splits to interpret what each component captures.

### 3.5 Summary of Models
 
| | Model 1 | Model 2 |
|---|---|---|
| Type | Supervised ensemble | Unsupervised reduction, then supervised |
| Technique | Random Forest (and Logistic Regression baseline) | PCA (k=3) plus Logistic Regression |
| Input features | 5 structured | 5 structured features, reduced to 3 principal components |
| Split | 70/15/15, seed 42 | 70/15/15, seed 42 |
| Metrics | AUC, F1 | AUC, F1 |
 
---
## 4. Results
 
Results mirror the Methods subsections. Interpretations are reserved for the Discussion section.
...
*Figure 2. Distribution of star ratings across all 109M reviews. The distribution is heavily skewed toward 5 stars (about 67.1M), with 1 star reviews second (about 9.4M), producing a J shaped pattern.*
...
*Figure 3. The ten most reviewed categories. Wireless leads (about 9M), followed by PC and Mobile Apps. Review counts are uneven across categories.*
...
*Figure 5. Higher star ratings correspond to higher helpfulness ratios (5 stars about 0.82, 1 star about 0.56).*
...
*Figure 7. Review volume rises from under 5M per year before 2011 to about 30M per year by 2015, while the average helpfulness ratio declines over the same period.*
...
*Figure 10. Class distribution of the binary label after preprocessing. Label 1 (helpful) totals 8,074,149 vs. 3,330,981 for label 0, an approximately 2.4 to 1 imbalance that motivates AUC and F1 as evaluation metrics rather than accuracy.*

### 4.3 Model 1 Results
AUC (area under the ROC curve) and F1 were used as primary metrics because the labels are imbalanced (label 1 is about 2.4 times label 0), under which raw accuracy would be misleading.
...
*Table 3. Three components retain 70.9% of total variance. The remaining 29.1% is discarded.*

<img width="600" height="250" alt="PCA Explained Variance (k=3, 5 input features)" src="https://github.com/user-attachments/assets/86c6098d-b9c6-4171-967c-c9472f292f0b" />
 
*Figure 14. Explained variance per component (left) and cumulative explained variance (right). The drop from 31.1% (PC1) to 20.3% (PC2) and 19.6% (PC3) is gradual, indicating no single dominant axis; three components reach 70.9% cumulative variance.*

Component loadings (sign indicates direction, magnitude indicates contribution):
 
| Feature | PC1 | PC2 | PC3 |
|---|---|---|---|
| `log_review_length` | -0.664 | -0.051 | 0.111 |
| `verified_purchase_idx` | -0.530 | 0.088 | -0.126 |
| `review_headline_length` | -0.489 | -0.284 | 0.250 |
| `star_rating` | -0.029 | 0.798 | 0.597 |
| `category_idx` | -0.197 | 0.522 | -0.744 |

*Table 4. PC1 is a composite "review effort" axis that loads strongly on the three text and engagement features. PC2 is dominated by `star_rating` (0.798) with a secondary contribution from `category_idx` (0.522), while PC3 is dominated by `category_idx` (-0.744) with a secondary contribution from `star_rating` (0.597). `star_rating` and `category_idx` load only weakly on PC1, so they carry variance that is largely independent of the three effort features.*

<img width="450" height="350" alt="PCA Component Loadings Heatmap" src="https://github.com/user-attachments/assets/aac7defd-7dc9-4b44-b45c-533ad9f36972" />

*Figure 15. PCA component loadings heatmap. PC1 loads negatively and strongly on `log_review_length` (-0.66), `verified_purchase_idx` (-0.53), and `review_headline_length` (-0.49). PC2 is driven by `star_rating` (0.80) and `category_idx` (0.52); PC3 is driven by `category_idx` (-0.74) and `star_rating` (0.60).*
 
<img width="1601" height="440" alt="PCA Projection Coefficients by Star Rating" src="https://github.com/user-attachments/assets/47f67a20-fe10-41fe-8f67-06d3d0cc0f40" />
...
| RF (n=20, d=15) | 0.7096 | 0.7060 | 0.7065 | 0.6871 | 0.6842 | 5 |
| RF (n=50, d=15) | 0.7098 | 0.7061 | 0.7068 | 0.6874 | 0.6845 | 5 |
| PCA (k=3) plus LR | 0.6347 | 0.6349 | 0.6347 | 0.6065 | 0.6068 | 5 to 3 |
 
*Table 5. PCA plus LR underperforms the full feature LR baseline by 0.0286 AUC and the best RF (n=50) by 0.0721 AUC.*
 
Predictions analysis (PCA plus LR, test set, 1,712,254 samples):
 
| Category | Count | Share |
|---|---|---|
| Correct (TP plus TN) | 1,218,728 | 71.2% |
| False Positives (predicted helpful, actually not) | 482,344 | 28.2% |
| False Negatives (predicted not helpful, actually helpful) | 11,182 | 0.7% |

*Table 6. The model is strongly biased toward predicting label 1. False Positives account for nearly all errors; False Negatives are near zero, consistent with the class imbalance (label 1 outnumbers label 0 by about 2.4 to 1) and the inability of a linear classifier to compensate for it after compression.*

### 4.5 Speedup Analysis (Distributed Computing)
 
The representative operation timed was the full train, transform, and count pipeline for RF (numTrees=20, maxDepth=15), run with 1 executor versus 15 executors on identical checkpointed data.
...
*Table 7. Measured distributed speedup.*
...
Reducing from five features to three components (70.9% variance retained) lowered performance at every stage. Test AUC fell from 0.6633 (full feature LR) and 0.7068 (best full feature RF) to 0.6347, an AUC gap of 0.0721 versus the best Random Forest. PC1 cleanly absorbs the three correlated effort features, but `star_rating` and `category_idx` carry variance that is largely independent of those features and is spread across PC2 and PC3, so truncating to three components removes discriminative signal rather than noise.
...
Using five structured metadata features, the best model was the Random Forest with numTrees=20 and maxDepth=15. It achieved a test AUC of 0.7065 and test F1 of 0.6842, improving on the logistic baseline by 0.043 AUC. RF (numTrees=50, maxDepth=15) scored marginally higher (test AUC 0.7068) but at substantially higher training cost for a gain of 0.0003, so the 20 tree model is preferred for its efficiency and quality trade-off on the full 50 GB dataset. The consistent train/val/test gap below 0.004 confirms good generalization, while the ceiling near 0.71 shows structured metadata alone provides a meaningful but limited signal.
...
Kayla Zhu: Writer/Coder: Wrote the project abstract; contributed Milestone 2 code (data exploration and 3 visualizations); wrote the Milestone 2 README; completed the majority of Milestone 3 code (preprocessing pipeline, trained and compared the logistic regression baseline and eight Random Forest configurations, example ground truth and predictions, fitting analysis, hyperparameter comparison, visualizations, and speedup analysis); wrote the entire Milestone 3 README; co-wrote the final Milestone 4 README; provided feedback throughout.

Hana Gamracy: Writer/Coder: Provided feedback to project abstract and choosing dataset, led the majority of the Milestone 2 code and write-up (Spark session configuration, data loading, exploratory analysis of star ratings, product categories, helpfulness ratios, and review length, and all accompanying visualizations); contributed to the Milestone 3 preprocessing pipeline and provided feedback throughout; co-wrote the final Milestone 4 README, provided feedback throughout.
...
Hyungwoo Kwon: Writer/Coder: Contributed to the questions that the group will handle; contributed to Milestone 2 code (data exploration and visualizations); allocated Milestone 3 preprocessing tasks and wrote the baseline notebook. Wrote the final Milestone 4 PCA code and prediction analysis.
